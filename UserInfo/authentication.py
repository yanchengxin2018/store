...
class Authentication:
    ...
    def authenticate(self,request,*args,**kwargs):
        try:
            ...
            if not self.valite_token(user_obj):return (None,'token已经失效了')
            return (user_obj,token)
        except:
            return (None,None)

    def valite_token(self,user_obj):
        if not user_obj:return False
        return True if (user_obj.validate_sign==self.data.get('validate_sign',None)) else False
